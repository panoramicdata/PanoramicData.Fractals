...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (hit) {
        ...
        // Calculate normal
        let eps = 0.0001;
        let xp = mandelbulb_de_full(p + vec3<f32>(eps, 0.0, 0.0), power, params.maxIterations).distance;
        let xn = mandelbulb_de_full(p - vec3<f32>(eps, 0.0, 0.0), power, params.maxIterations).distance;
        let yp = mandelbulb_de_full(p + vec3<f32>(0.0, eps, 0.0), power, params.maxIterations).distance;
        let yn = mandelbulb_de_full(p - vec3<f32>(0.0, eps, 0.0), power, params.maxIterations).distance;
        let zp = mandelbulb_de_full(p + vec3<f32>(0.0, 0.0, eps), power, params.maxIterations).distance;
        let zn = mandelbulb_de_full(p - vec3<f32>(0.0, 0.0, eps), power, params.maxIterations).distance;
        
        let normal = normalize(vec3<f32>(xp - xn, yp - yn, zp - zn));
        ...
        let diffuse2 = max(dot(normal, lightDir2), 0.0) * 0.5;
        let ambient = 0.3;
