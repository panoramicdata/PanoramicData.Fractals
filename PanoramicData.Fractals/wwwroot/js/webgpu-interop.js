import { getShaderCode } from './fractal-shaders.js';
...
async function createComputePipeline(fractalType) {
	...
	try {
		// Load shader code from .wgsl file
		const shaderCode = await getShaderCode(fractalType);
		console.log(`Loaded shader code for ${fractalType} (${shaderCode.length} characters)`);
